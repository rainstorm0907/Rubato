...
- **headless-seat-can-orchestrate** — 헤드리스 경로(`rubato dispatch` 등)로 띄운 owner 가 자기 helper 를 필요로 함. → 하네스의 spawn 표면이 있다고 읽고 쓴다. "비대화형에는 subagent 가 없다"는 부정형 기록으로 되돌아가지 않는다 (2026-08-21 실측: 헤드리스 세션이 `subagent.create` + `inspect.wait` 로 자식을 만들어 결과를 회수).
...
- **no-standing-fable-seat** — 사용자 경험이 중요한 기능의 팀 구성. → Fable은 사용자가 선택한 framing 구간에서만 쓰고, 상시 teammate로 자동 추가하지 않는다.
