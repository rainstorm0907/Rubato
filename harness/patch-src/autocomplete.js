...
export class CombinedAutocompleteProvider {
    ...
    async getSuggestions(lines, cursorLine, cursorCol, options) {
        ...
        if (dollarContext) {
            const suggestions = getDollarInvocationSuggestions(this.commands, dollarContext.query, dollarContext.skillsOnly);
            if (suggestions.length === 0)
                return null;
            return {
                items: suggestions,
