...
> **모델이 갑자기 느려졌다면** 하네스 비교가 아니라 `bridge-latency.mjs` 다.
> 아래 "브리지가 느려졌을 때" 절로 간다.
...
node bridge-latency.mjs --model xai/grok-4.6 --rounds 6
node bridge-latency.mjs --raw            # 브리지를 빼고 xAI 직접 (대조군)
```

동시 요청을 라운드로 반복해 **계단인지 아닌지**를 가른다. 그 갈림이 먼저다.

| 결과 | 뜻 | 다음 |
|---|---|---|
| 라운드가 갈수록 뒤 요청이 밀린다 (3/9/13/16초) | 연결이 하나로 수렴해 요청이 직렬화됐다 | `bridge/src/upstream-dispatcher.ts` |
| 다 같이 느리다 | 프로바이더나 네트워크 | `--raw` 로 브리지를 빼고 대조 |
| 하나만 툭 튄다 | 단발 꼬리. 미해결 항목 | 케이스스터디의 "미해결" 참고 |

**`--raw` 가 핵심 대조군이다.** 브리지를 완전히 빼고 같은 라운드를 도는데,
여기서도 계단이 서면 브리지 코드는 무죄다. 2026-08 진단에서 "프로세스가
상했다" 와 "브리지가 직렬화한다" 를 두 번 다 반증한 게 이 대조였다.

**`rbr` 재시작이 잘 들으면 그것도 신호다.** 예전에 재시작이 들었던 건 프로세스가
건강해져서가 아니라 큐가 비어서였다.

배경과 미해결 항목:
`agent-taskforce/case-studies/harness/bridge-http2-connection-reuse-serialization.md`
