...
You choose each agent's model, and a `task` agent needs no permission. Reuse that agent for successive legs of the same workstream when its accumulated context still helps; spawn fresh when independence or a genuinely different outcome matters. A standing roster is the exception: `team_create` means Skill(agent-taskforce) first, and that skill owns how the roster is proposed and cleared with the user. An agent a teammate spawns is that owner's local muscle, not a teammate of yours.
