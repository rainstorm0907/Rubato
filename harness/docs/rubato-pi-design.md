...
| 17 | `memory` | **프로젝트 지식을 세션 밖에도 저장해 다음 대화에 다시 넣는다.** 도구 2개, 슬래시 명령 13개, HTML 뷰어를 제공하고, 쉬는 동안 별도 자식 프로세스가 메모리를 정리하는 "dreaming"도 실행한다. `memory.enabled` 기본 ON | **지금은 작다.** 예전엔 `before_agent_start` 마다 컴파일된 블록을 통째로 합성했고(이 기기 기준 30KB), 지금은 `memory.project` 화이트리스트에 적힌 파일만 실린다. 기본값은 빈 목록이라 AGENT_ID 한 줄이고, 나머지는 msearch 로 찾아 읽는다. `compile_warn_tokens` 기본 30000 | 자식 프로세스는 여전히 배경에서 뜨다(브리지를 거치게 만들었다). 프롬프트 접두가 커밋 HEAD에 따라 흔들던 문제는 빈 화이트리스트에서는 사라졌다 | 기본 OFF — 8절의 캐시·비용 지표를 측정할 때까지 | **유지** |
