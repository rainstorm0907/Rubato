...
fx의 subagent는 원래 부모-자식 트리다. 자식은 부모에게만 말할 수 있고, 자식끼리는 서로를 볼 수도 부를 수도 없었다. 통신을 막고 있던 것은 자료구조가 아니라 **권한 위상**이었다 — 메시지 봉투는 이미 `source_id`, `target_id`, `work_id`를 따로 갖고 있었지만, 대상 검사가 "내게 붙어 있는 자손인가"만 물었다.
...
리드를 자동으로 깨우는 선택지는 버렸다. root 세션이 사용자 없이 새 턴을 시작하면 승인 경계가 무너지고, "리드는 백그라운드 자식이 아니다"라는 전제와 어긋난다.
